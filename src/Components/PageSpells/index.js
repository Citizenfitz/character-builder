...
const spells = Object.entries(spellData).map(([spellName, spell]) => ({...spell, ...{ name: spellName }}));
const filterConfig = [
  {
          ...
          { filterConfig.map(c => (
            <Fragment key={c.filterType}>
              ...
              <input
                ...
                onChange={() => {setDisplayedTitle(c.label); setFilterType(c.filterType)}}
              />
              <label htmlFor={c.filterType}>{c.dotColor ? <span className={`fas fa-spell-dot fa-spell-dot--${c.dotColor}`}></span> : ''} {c.label}</label>
            </ Fragment>
          ))}
          <br />
        </fieldset>
...
function levelFilter(prop) {
  return (arr) => arr.filter(s => s[prop] > 0).sort((a, b) => a[prop] - b[prop]);
}

function colorFilter(color) {
  return (arr) => arr.filter(s => s.school?.includes(color)).sort(spellAlphaSort)
}